...
public class MyLevel extends Level{
	...
	public void create(MyDNA dna, int type)
	{
        ...
        int currentWidth = STARTOFFSET;
        int stringIndex = 0;
        char charAt;
        Random rand = new Random();
        while (currentWidth < width - EXITOFFSET) {
            if(stringIndex >= dna.getLength()) {
                charAt = dna.getChromosome().charAt(rand.nextInt(dna.getLength()));
            } else {
                charAt = dna.getChromosome().charAt(stringIndex);
            }

            if(charAt == 'j') {
                currentWidth += buildJump(currentWidth, width-EXITOFFSET-currentWidth);
            } else if (charAt == 'c') {
                currentWidth += buildCannons(currentWidth, width-EXITOFFSET-currentWidth);
            } else if (charAt == 'h' && width-EXITOFFSET-currentWidth > 10) {
                currentWidth += buildHillStraight(currentWidth, width-EXITOFFSET-currentWidth, rand.nextInt(50));
            } else if (charAt == 't') {
                currentWidth += buildTubes(currentWidth, width-EXITOFFSET-currentWidth, rand.nextInt(50));
            } else if (charAt == 's') {
                currentWidth += buildStraight(currentWidth, width-EXITOFFSET-currentWidth, false, rand.nextInt(50));
            } else if (charAt == 'g' && width-EXITOFFSET-currentWidth > 10) {
                currentWidth += buildHillStraightNoEnemies(currentWidth, width - EXITOFFSET - currentWidth);
            } else if (charAt == 'a') {
                currentWidth += buildStraightNoEnemies(currentWidth, width-EXITOFFSET-currentWidth, false);
            } else if (charAt == 'q') {
                currentWidth += buildStraightNoCoins(currentWidth, width-EXITOFFSET-currentWidth, false, rand.nextInt(50));
            } else {
                currentWidth += buildStraightNoCoinsNoEnemies(currentWidth, width-EXITOFFSET-currentWidth, false);
            }
            stringIndex++;
        }
    ...
    //A built in function for building a flat hill
    public int buildHillStraightNoEnemies(int xo, int maxLength)
    {
        if (maxLength < 10) {
            throw new IllegalArgumentException("Hill length must be at least 10.");
        }
        ...
        while (keepGoing)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (random.nextInt(4) == 0)
                    {
                        decorateNoEnemies(xxo - 1, xxo + l + 1, h);
                        keepGoing = false;
                    }
                    for (int x = xxo; x < xxo + l; x++)
                    {
    ...
    //A built in function for building a straight path
    public int buildStraightNoCoins(int xo, int maxLength, boolean safe, int difficulty)
    {
        int length = random.nextInt(10) + 2;
        ...
        if (!safe)
        {
            if (length > 5)
            {
                decorateNoCoins(xo, xo + length, floor, difficulty);
            }
        }
    ...
    //A built in function for building a straight path
    public int buildStraightNoCoinsNoEnemies(int xo, int maxLength, boolean safe)
    {
        int length = random.nextInt(10) + 2;
        ...
        /*if (!safe)
        {
            if (length > 5)
            {
                decorateNoCoinsNoEnemies(xo, xo + length, floor);
            }
        } */
    ...
    //A built in function for building a straight path
    public int buildStraightNoEnemies(int xo, int maxLength, boolean safe)
    {
        int length = random.nextInt(10) + 2;
        ...
        if (!safe)
        {
            if (length > 5)
            {
                decorateNoEnemies(xo, xo + length, floor);
            }
        }
    ...
    //A built in function to add "decoration"
    public void decorateNoCoins(int xStart, int xLength, int floor, int difficulty)
    {
        //if its at the very top, just return
        if (floor < 1)
            return;
    ...
    //A built in function to add "decoration"
    public void decorateNoCoinsNoEnemies(int xStart, int xLength, int floor)
    {
        //if its at the very top, just return
        if (floor < 1)
            return;
    ...
    //A built in function to add "decoration"
    public void decorateNoEnemies(int xStart, int xLength, int floor)
    {
        //if its at the very top, just return
        if (floor < 1)
            return;
