...
public class MyLevelGenerator{
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		String[] profiles = new String[] {"j", "c", "h", "t", "s", "a", "g", "q", "n"};
		String returnString = new String();
		int randIndex = rand.nextInt(profiles.length);
		for(int i = 0; i < 20; i++) {
			returnString += profiles[randIndex];
			randIndex = rand.nextInt(profiles.length);
		}
		individual.setChromosome(returnString);
	...
	private boolean terminate (ArrayList<MyDNA> population, int count)
	{
		...
		// YOUR CODE GOES BELOW HERE
		if ((this.getBestIndividual(population).getFitness() > 0.8) || (count > 5000)) {
			decision = true;
		}
		// YOUR CODE GOES ABOVE HERE
		return decision;
	...
	private ArrayList<MyDNA> selectIndividualsForMutation (ArrayList<MyDNA> population)
	{
		...
		for (int i = 0; i < this.getPopulationSize()/2; i++) {
			randIndex = rand.nextInt(getPopulationSize());
	...
	private ArrayList<MyDNA> globalCompetition (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation)
	{
		ArrayList<MyDNA> finalPopulation = new ArrayList<MyDNA>();
		// YOUR CODE GOES BELOW HERE
		ArrayList<MyDNA> compositeList = new ArrayList<MyDNA>();
		compositeList.addAll(oldPopulation);
		compositeList.addAll(newPopulation);
		Collections.sort(compositeList, (a,b) -> b.compareTo(a));
		for (int i = 0; i < this.getPopulationSize(); i++) {
			finalPopulation.add(compositeList.get(i));
		}
		// YOUR CODE GOES ABOVE HERE
		if (finalPopulation.size() != this.getPopulationSize()) {
			throw new IllegalStateException("Population not the correct size.");
